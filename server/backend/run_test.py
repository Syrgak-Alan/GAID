...
from .gAIde import recognize_showplace_auto


def get_coordinates() -> Dict[str, float]:
    return {"latitude": 48.147693, "longitude": 1.569697}


coords = get_coordinates()
lat, lon = coords["latitude"], coords["longitude"]

def describe_place(image, lat=lat, lon=lon):
    list = recognize_showplace_auto(image, lat=lat, lon=lon)
    print(list)


describe_place("/Users/emir/Desktop/rrr/GAID/server/backend/alte_pinakothek.jpg", lat=lat, lon=lon)